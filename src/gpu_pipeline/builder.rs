...
pub struct PipelineBundle {
    ...
    // Shared reference-orbit buffers (vec2f / Complex32 per point). The FExp
    // shader reads these same f32 buffers and lifts each point to FExp on load —
...
fn build_shader_calc_pipeline(
    ...
    let rank_one_orbit_buf = device.create_buffer(&wgpu::BufferDescriptor {
        ...
        size: (settings.max_ref_orbit as usize * size_of::<Complex32>()) as u64,
        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_DST,
        mapped_at_creation: false,
    });
    ...
    let calc_burningship_pipeline =
        device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("calc_burningship_pipeline"),
            layout: Some(&calc_pipeline_layout),
            module: &calc_burningship_shader,
            entry_point: Option::from("main"),
            compilation_options: Default::default(),
            cache: None,
        });
